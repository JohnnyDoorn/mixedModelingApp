...
Model 1) Intercept ($\mu$) only, there are no differences across conditions and participants.
\[
Y_{ijm} \sim \mbox{N}(\mu,\sigma\epsilon^2)
\]


Model 2)  Fixed effect $\nu$ of condition, which is the same for all participants. The term $x_j$  indicates the specific condition (i.e., $x_j= -\nicefrac{1}{2},\nicefrac{1}{2}$ if $J = 2$), which ensures the sums-to-zero constraint.
\[
Y_{ijm} \sim \mbox{N}(\mu+x_j\nu,\sigma\epsilon^2)
\]

Model 3) No fixed effect of condition, but individual differences in baseline. The random intercepts are denoted with $\alpha_i$, where the subscript indicates that the intercepts are specific to each participant. The random intercepts vary around the intercept $\mu$, with standard deviation $\sigma_\alpha$. Such random intercepts can also be seen as a main effect of participant.  When $\sigma_\alpha$ is set to 0, Model \ref{eqn:mod3} is equivalent to Model \ref{eqn:mod1}. In one-way RM ANOVA, this model is typically used as the null model.
\[
\begin{aligned}
